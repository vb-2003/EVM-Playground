...
import { Button, Input, Icon, Message, Toggle } from 'components/ui'
...
const Editor = ({ readOnly = false }: Props) => {
  ...
  const [forkSlotInput, setForkSlotInput] = useState('0x0')
  ...
  const handleForkToggle = (event: React.ChangeEvent<HTMLInputElement>) => {
    const enabled = event.target.checked
    setForkConfig({
      enabled,
      status: enabled ? forkConfig.status : 'idle',
      error: enabled ? forkConfig.error : undefined,
    })
  }

  const handleLoadForkedContract = useCallback(async () => {
    const result = await loadForkedContract()
    if (!result) {
      return
    }

    setCodeType(CodeType.Bytecode)
    setCodeModified(true)
    setCode(result.bytecode)
    setIsExpanded(false)
    setContract(undefined)
    setMethodByteCode(undefined)
    loadInstructions(result.bytecode)
  }, [loadForkedContract, loadInstructions])

  const handleFetchStorageSlot = useCallback(async () => {
    if (!forkSlotInput) {
      return
    }
    await fetchForkStorageSlot(forkSlotInput)
  }, [fetchForkStorageSlot, forkSlotInput])
  ...
  const handleRun = useCallback(() => {
    ...
    try {
      ...
      if (forkConfig.enabled && forkedContract?.bytecode) {
        const address = createAddressFromString(forkedContract.address)
        loadInstructions(forkedContract.bytecode)
        startExecution(
          forkedContract.bytecode,
          _callValue,
          _callData,
          address,
        )
